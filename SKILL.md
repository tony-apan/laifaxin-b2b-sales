...
flowchart TD
    ...
    N --> S0["S0: AI出ABCD客群方案<br/>用户选字母"]
    S0 --> S3["S3: 用户有认得的买家网址?<br/>有→填入;没有→标准路径,不追问"]
...
- **★最小必要输入（2026-09-03 用户拍板：渐进索取，禁止开局列清单）**：
  ...
  - **后续节点用到现在才要**：S2 客群确认时给 A/B/C/D 方案让用户选（不逼用户描述画像）；S3 用户可给一个认得的买家网址（没有就走标准路径，不追问）；S7 若正文要写公司名/官网/邮箱等署名信息，**先看用户是否已主动提供**——提供了就用，没提供就不写这些字段（落款只用昵称），**禁止主动列清单索要**。
...
| 节点 | 一句话判据 |
|------|-----------|
| S0 INPUT_GATE | **只需昵称+一句话产品**（中英皆可）。★S0 出 A/B/C/D **获客方向方案**（含推荐与淘汰理由），用户选字母——禁止开局索要公司名/官网/邮箱/认证/MOQ 清单 |
