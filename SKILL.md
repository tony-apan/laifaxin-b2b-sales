...
- **★最小必要输入（2026-09-03 用户拍板：渐进索取，禁止开局列清单）**：
  ...
  - **后续节点用到现在才要**：S0 出 A/B/C/D 方案选字母；S2 出具体客群表选编号（两步分工，不重复问）；S3 用户可给一个认得的买家网址（没有就走标准路径，不追问）；S7 若正文要写公司名/官网/邮箱等署名信息，**先看用户是否已主动提供**——提供了就用，没提供就不写这些字段（落款只用昵称），**禁止主动列清单索要**。
