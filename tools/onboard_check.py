...
print("★ 会话入口总线: ①bootstrap(无Python环境准备) → ②本脚本(自检+可续接项目扫描) → ③S0a公司/产品档案+适配判断 → ④首次平台调用前check_login → ⑤gate_check闸门 → ⑥flow_orchestrator向导")
print("")
print("🔧 环境自检(30秒):")
...
if projects:
    ...
    print(f"     → {python_cmd or 'python3'} tools/check_login.py --token '<一键双取两行整段>' 复验 → bash tools/gate_check.sh --token '<同一整段>' --product <项目键> 过闸")
    ...
    print("\n✅ 引导完成。AI 请按序执行:")
    print(f"  1. 先向用户要纯个人昵称+一句话产品（可选补目标市场）；不要开局催 token")
    ...
    print(f"  4. 首次调用来发信平台前，再向用户获取 token（教程 https://www.laifa.xin/share/ai/laifaxin-ai-account-connection）")
    print(f"     → {python_cmd or 'python3'} tools/check_login.py --token '<一键双取两行整段>'   ← 登录检查+账号状态卡")
    print("  5. bash tools/gate_check.sh --token '<同一整段>' --product <operator_key>/<product_key>  ← 闸门")
    ...
    print('   "您先给我一个纯个人昵称,再说一句卖什么;我先了解产品并判断是否适合,剩下的我来。"')
    print('   "卖给谁、卖到哪都可以不填,我会先给推荐;如果有您自己的官网、产品页或目录,也可以发给我读取提炼,没有就直接继续。"')
