...
def clist(pg):
    ...
    try:
        ...
        data=d.get("data")
        if not isinstance(data,dict) or not isinstance(data.get("list"),list): raise ValueError("返回缺list")
        return data["list"]
    except Exception as exc:
        print(f"❌ 第{pg}页查询失败，无法证明排除生效: {exc}")
        sys.exit(1)
bad=[]
for pg in [int(x) for x in args.pages.split(",")]:
    for c in clist(pg):
        ...
        if not cc:
            print(f"❌ p{pg} 记录缺country_code，无法证明排除生效")
            sys.exit(1)
