...
| 🗺️ [一张图看懂全流程](#how-it-works) | 找买家 → 12轮触达 → 询盘 → 背调跟进 |
| 📖 [完整方法：广撒网 + 精准跟进](docs/09-mass-outreach-to-precision-follow-up.md) | 成本账 / 询盘背调 / 分级 / 多渠道 |
| **💰 投入前** | |
| 🎫 [门票与验证闭环](#-门票与最小验证闭环先看这个再投入) | 先免费 15 天 SVIP 试 · 投入前 48 小时用询盘数裁决 |
| **🔄 老用户** | |
| 🔄 [更新到新版本](#-更新到新版本老用户) | 同一台电脑升级（保留本地数据）|
| 💻 [换电脑继续干](#-换电脑继续干老用户) | 迁移本地状态，从当前节点接着跑 |

> 💡 **可以先零成本试**：联系来发信客服**免费领 15 天 SVIP**，不用先花一分钱；跑通觉得有价值，再考虑投入——投入前怎么算账、48 小时怎么裁决，见 [🎫 门票与最小验证闭环](#-门票与最小验证闭环先看这个再投入)。
...
**适合你，如果你**：缺少客户基数、想先批量验证一个市场；不想把大量人工时间耗在没有回复的陌生公司上；有能力承接询盘后的背调、报价、样品、谈判。

| 产品形状 | 举例与原因 |
|---|---|
| ✅ **更适合**：买家多、决策快、有渠道商 | 保温杯、户外用品、标准工业组件——同行买家成千上万、习惯邮件询价，还有批发/分销/集成商可打 |
| ⚠️ **不太适合**：大宗原料、长周期项目制、需招标/认证/入围 | 回询盘以月甚至年计——广撒网只能低成本摸底，不能替代展会、渠道或关键客户开发 |

判断口诀：**你的买家盘子里，能成批检索到多少家？他们多久采购一次？谁会用邮件回你？**（AI 会在开跑前帮你判三档并如实说明，不耽误你试；细节见 [specs/product-fit.md](specs/product-fit.md)）

## 🎫 门票与最小验证闭环（先看这个再投入）

| 步骤 | 内容 |
|---|---|
| 0️⃣ **零成本试** | 联系来发信客服，**免费领 15 天 SVIP** |
| 1️⃣ **门票** | 觉得有价值再上 **399 元/年 SVIP**（含赠点约 2.3w，价格以平台实时为准）——够保存约 1~2 万条买家邮箱 |
| 2️⃣ **一波** | 保存约 1w 联系人 → 12 轮自动跟进（总发送约 12w 封·跨约 10 个月） |
| 3️⃣ **裁决** | 首波发出后 **48 小时**，数有效询盘（同行套价/自动回复不算） |

```mermaid
flowchart TD
    A["0️⃣ 免费领 15 天 SVIP"] --> B["1️⃣ 399 元/年 SVIP<br/>赠点约 2.3w（以平台为准）"]
    B --> C["2️⃣ 一波验证<br/>存约 1w 联系人·12 轮跟进"]
    C --> D{"发出后 48 小时<br/>有效询盘数？"}
    D -- "≥3 个·合格" --> E["扩量：询盘方网址=新种子<br/>继续找相似"]
    D -- "<3 个·偏低" --> F["诊断漏斗：送达→打开→回复<br/>换客群/品类再试一轮"]
    F --> C
    F -. "连续两轮不达标" .-> G["❌ 停止该项目（止损）"]
```

> 询盘≠订单；每步你确认、激活必须你亲口下令——责任设计不变。
...
<details>
<summary><b>🔧 给技术朋友（附录：运行环境 / 目录导览 / 合规与安全边界）</b>——默认收起，点开查看</summary>
...
<details>
<summary><b>办法 A（推荐，快又稳）：点开复制完整更新指令，整段发给 AI</b></summary>
...
<details>
<summary><b>💻 旧电脑：点开复制备份指令，整段发给 AI</b></summary>
...
<details>
<summary><b>🖥️ 新电脑：装好最新版后，点开复制续接指令，整段发给 AI</b></summary>
...
许可证：GPL-3.0 · [更新记录 CHANGELOG](CHANGELOG.md) · 反馈：[GitHub Issues](https://github.com/tony-apan/laifaxin-b2b-sales/issues)
