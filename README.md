...
> **你只要告诉 AI：卖什么、卖给谁、卖到哪。它按流程帮你找买家、检查名单、写开发信并排好跟进；收到询盘后，你再接手报价和成交。**
...
| 🗺️ **先看它怎么工作** | [一张图看懂完整流程](#how-it-works) |
...
请使用**能在你的电脑上打开文件、运行命令的 AI 助手**（不是只能聊天的普通 AI），例如 ZCode、Claude Code 或 Codex。把下面整段复制给它，只需把 `XXX` 换成你的产品：

```text
请帮我安装并带我使用这套外贸获客系统：
https://github.com/tony-apan/laifaxin-b2b-sales

请下载并通读仓库，严格按仓库规则自动准备环境、安装和自检；
不要跳过安全检查，也不要未经我确认执行会改动账号的操作。
完成后用通俗中文告诉我它能做什么、哪些动作需要我确认，再按流程带我操作。

我卖的是：XXX
目标市场：XXX（不确定可以先问我）
token 现在不用催，需要连接来发信账号时再一步步带我获取。
```

**发完就等 AI 带你走。** 你不需要先学 Git、Python、token，也不用自己敲命令。

接下来只会发生四件事：

1. AI 安装并自检这套系统；
2. AI 了解你的产品，判断它是否适合批量冷邮件；
3. 需要时，AI 带你连接来发信账号并找买家；
4. 保存、建序列、添加联系人、激活发信等关键动作，AI 都会先向你确认。
...
```mermaid
flowchart TD
    U["你先告诉 AI<br/>卖什么·卖给谁·卖到哪"] --> A["AI 判断产品是否适合<br/>并了解产品信息"]
    A --> B["需要时连接来发信账号<br/>再获取账号钥匙"]
    B --> C["找买家并抽查名单"]
    C --> D{"你确认保存名单？"}
    D -- "要调整" --> C
    D -- "确认" --> E["保存名单"]
...
AI 会按顺序了解你的信息：开局只问你的纯个人昵称和卖什么；之后再分两轮询问公司资料和产品资料（官网、目录、卖点、认证、产能、最低起订量、交期等）。**这些补充资料都可以跳过，不会在开局让你填一张长表。** 邮件末尾签名区永远只有纯个人昵称；经你确认且有来源的产品事实，才可用于正文卖点。
...
- token 等同账号钥匙，只发给你信任且正在本机操作的 AI，不发群聊、工单或公开网页，也不写入仓库文件。
