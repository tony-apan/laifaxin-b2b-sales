...
| L-55 | 全局文案替换污染代码/命令区（跨副本一致性被破坏） | 🔴 高 | 已修复 | 文档 |
| L-56 | 文档引用与检查不得依赖"行号/同一行" | 🔴 高 | 已修复 | 文档 |
| L-57 | 大改文档前先建"内容保真基线"（去空白 sha） | 🔵 中 | 已固化 | 文档 |
| L-58 | 工作空间靠 HTTP header `uid` 传，query `?uid=` 无效 | 🔴 高 | 已修复 | 连接 |
| L-59 | 换产品即失效的工具（硬编码词表）+ 单页非单调 | 🔴 高 | 已修复 | 审计 |
| L-60 | 写操作工具必须幂等（重跑不报假失败） | 🔴 高 | 已修复 | 模板 |
| L-61 | 对外事实（价格/赠点/权益）必须核对官网原文，不得凭记忆 | 🔴 高 | 已修复 | 文档 |

> 当前共 **61 条**（L-01~L-61；L-44 起为脱敏抽象条目，随库分发）。
...
### L-55 🔴 全局文案替换污染代码/命令区（脱敏抽象，2026-09-10）

- **问题**：做"对用户统一用敬称「您」"的全局替换时，把**一键双取命令内部的文案**也改了（命令里含 `❌ 你现在打开的网页…`、`有没有你的账号头像`）。该命令要求**多副本逐字一致**（用户复制后粘到浏览器控制台，一处不同即可能行为不一致），替换后 6 份副本裂成 2 个版本。
- **发现**：拆行重构时顺手核对"命令完整性"，比对六副本 sha 才发现（此前多版都没查过）。
- **教训**：⚠️ **批量替换必须先界定作用域**——文案规范只作用于"用户话术"，**代码/命令/配置区是不同域，必须显式排除**；同一字符串在仓库里可能同时存在"要改的文档副本"和"不能改的代码副本"，全局 `replace` 天然会误伤后者。
- **预防**：①替换前先 grep 目标串**全部出现位置**并分类（文档 / 代码 / 命令）；②对"要求逐字一致"的资产建**跨副本一致性断言**（本次已加 `OneClickCommandConsistencyTest`：扫描全仓该命令，必须字节一致 + 单行 + 保留原文用词）；③替换后立刻跑该断言。

---

### L-56 🔴 文档引用与检查不得依赖"行号 / 同一行"（脱敏抽象，2026-09-10）

- **问题**：规则文档大量用**行号引用**指路（`RULES.md L27`、`API L54`、`threshold-method L30-36`）。每次往文档加内容，行号整体后移——实测 **10 处 RULES 行号引用全部失效**，`API L260` 甚至指向了无关接口（引用处讲 sequence-create，实际是 sequence-details）。AI 照行号去查会拿到**完全错误**的规则，比不写引用更糟。
- **同类**：`gate_check.sh` 的闸门用**同行正则** `签名区.*只有昵称`——文档换行/拆行后该行不再匹配，闸门会**误报"缺规则"**（实测拆行前必须先把闸门改成 token 独立检查）。
- **教训**：⚠️ **文档里的"位置"引用天然脆弱**——行号、同一行内的多词匹配都会随排版变化失效；**检查项应与排版解耦**。
- **预防**：①引用一律用**稳定标识**：节点名（`RULES.md` S4）、章节名（「客群输出标准」）、接口名（见 api-reference 接口表）；②闸门多关键词用**token 独立 grep** 而非同行正则；③加测试禁止行号引用（保留教训编号 `L-45`、目录编号 `L0`-`L3` 等合法形态）——本次已加 `NoLineNumberReferenceTest`。

---

### L-57 🔵 大改文档前先建"内容保真基线"（脱敏抽象，2026-09-10）

- **场景**：要把规则文档里最长 1154 字符的超长行（12 个子条款挤一行）拆成多行。拆行**只应改变空白分布**，不得增删任何字符——但人工核对上千字不现实。
- **做法**：拆前计算 **"去掉所有空白后的字符序列 sha256"** 作为基线；拆后重算比对。相同 = 零丢失。
- **结果**：三个文件（RULES/SKILL/node-playbook）拆行后 sha 与拆前**完全一致**，证明"只动了排版"。
- **教训**：⚠️ 结构性重构（拆行/重排/格式化）必须用**归一化指纹**证明"语义未变"，不能靠"看起来一样"；归一化要选对维度（此处去空白）——否则会掩盖真实丢失。
- **预防**：任何整篇重排/迁移/格式化前，先记归一化 sha 基线，改后比对；不通过则回滚重来。

---

### L-58 🔴 工作空间靠 HTTP header `uid` 传，query `?uid=` 无效（脱敏抽象，2026-09-09 真机双空间实测）

- **现场**：真机跑企业空间时，工具传 `?uid=<企业ID>` 却**始终操作个人空间**（余额/联系人/序列全部对不上）。旧认知是"平台按 uid 路由、无权限时静默回落"——据此做的校验方向也偏了。
- **决定性对照（同一 token、同一 orgId）**：header `uid: <企业ID>` → `isOrg=true`、余额 4,661,777、联系人数十万；query `?uid=<企业ID>` → `isOrg=false`、余额 768,265、联系人 11,572。**且 query 传任意值（含不存在的 ID）都返回个人空间数据**——证明平台**根本不读** query 里的 uid。
- **影响**：所有写操作都会落到 token 绑定的空间，`--org` 传什么都无效；历史事故"给了企业 orgId 却写进个人账号"根因即此。
- **补救**：22 个调用点全部改为 header `uid`（curl 加 `-H "uid: <org>"`、urllib 加 `headers={"uid": org}`）；query 保留仅作兼容。
- **教训**：⚠️ **"参数传了" ≠ "平台采纳了"**——对接第三方 API 时必须用**对照实验**验证参数是否真的生效（改变量看输出是否跟着变；传一个明显无效的值看是否被拒），不能只看"接口返回 success"。
- **预防**：①工作空间类参数一律同时进 header；②对"要求生效"的参数做**反向验证**（传坏值应报错，不报错说明没生效）；③加测试断言所有 API 工具都发 header uid，且 uid 取 org 而非 token。

---

### L-59 🔴 换产品即失效的工具（硬编码词表）+ 单页精准度非单调（脱敏抽象，2026-09-09 真机）

- **问题1 硬编码词表**：审计工具内置判定词表是「宠物食品包装」样例，换产品（皮筏艇）不传产品词 → **全页判 0% 精准度**（同一批数据：不带词 0%、带词 100%）。工具**静默给 0%** 而无任何提示——AI 会据此误判"该页没有目标买家"。
- **问题2 单页非单调**：实测单页精准度**非单调**（700页80% → 710页30% → 730页70% → 750页40%），若只看单页会误判"跌破了要往前退"；文档虽已规定"三页滑动平均"，但缺真机案例，AI 仍易按单页判。
- **补救**：①工具 fail-fast 强制要求产品词（缺失即退出，不再静默给 0%）；②补真机案例强化"禁止用单页定临界"。
- **教训**：⚠️ **写死业务词表的工具=换场景即失效**，必须把词表变成"调用方必传参数"并在缺失时**报错而非返回零结果**；⚠️ **有噪声的指标必须规定聚合口径**（这里是三页平均）并配真机反例，否则规定形同虚设。
- **预防**：①任何"打分/判定"类工具，缺关键输入时 fail-fast；②指标类规则必须带**真实数值案例**说明为什么不能用单点。

---

### L-60 🔴 写操作工具必须幂等（重跑不报假失败）（脱敏抽象，2026-09-09 真机）

- **现场**：批量创建资源（120 模板）时，上一次运行因**另一个错误**中止，但资源**已实际写成**；重跑时报"名称已存在"并整体失败——工具把"已成功"报成了"失败"，会误导 AI 去**删数据重来**（浪费且有风险）。
- **同类**：分组复用逻辑只读 `id` 字段，但接口返回的是 `_id` → 拿到空值 → 误判"无此分组" → 重复创建撞重名 → 中止。
- **补救**：①创建前查同名，存在则**复用其 id**（幂等）；②字段取值兼容 `id`/`_id`，但**不得误取父目录字段**（`foid` 常为 `"0"`）。
- **教训**：⚠️ **写操作工具必须幂等**——"已存在"应当是**成功的一种**（复用），不是失败；⚠️ **报错文案必须准确**：把"已成功"报成"失败"比报错本身危害更大（会诱导破坏性操作）。
- **预防**：①创建类工具统一"查同名→复用/新建"；②中止信息区分"本次没做成"与"上次已做成"；③字段名不要猜，从真实响应里取值。

---

### L-61 🔴 对外事实（价格/赠点/权益）必须核对官网原文，不得凭记忆（脱敏抽象，2026-09-09）

- **现场**：仓库写"套餐赠点约 2.3 万"，核对官网权益页后**该数字不存在**——官网写的是"购买后到账 20,000 点（永久）+ 每月赠 3,000 点"，且价格口径也不完整（缺原价/券后价对照）。另一处：**全仓没有"如何注册账号"的引导**，用户没账号时会卡在连接那一步。
- **教训**：⚠️ **对外事实会过期**，凭记忆或"上次看到"写入文档=埋雷；必须**抓取官网原文**核对，且所有价格/权益声明都要带**"以平台实时为准"免责**。
- **预防**：①价格/权益/政策类数字，写入前抓官网核对；②声明处统一带免责口径；③补"用户从零开始"的前置引导（没账号怎么办），别默认用户已经有账号；④加测试禁止过时数字复现、强制免责存在。
