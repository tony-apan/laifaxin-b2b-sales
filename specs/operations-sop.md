...
**铁律（`RULES.md` 7d）**：每个被选中的客群**独立成档**——独立一对标签、独立一套 120 模板、独立一条序列；**禁止合并**。
...
| 客群 | 标签 | 模板批次 | 序列 |
|---|---|---|---|
| <客群A：如 景区运营商> | `<tagA(名称)>` | <批次前缀A>（120，运营商角度） | `-S04` |
| <客群B：如 装备批发商> | `<tagB(名称)>` | <批次前缀B>（120，经销利润/MOQ/条款） | `-S05` |
| <客群C：如 装备零售商> | `<tagC(名称)>` | <批次前缀C>（120，零售上架/动销/小单） | `-S06` |
...
对每个选中客群，按用户确认的顺序依次走完整条链：
  ...
  → S7/S8 生成该客群 120 模板 → S9 建该客群序列 → S10 加该客群联系人
一个客群全部走完，再做下一个；不得并行铺开、不得合并写操作。
```

**确认节点必答项**：
- S2 选中 ≥2 个客群 → **必须**提示"建议分开做"并给出"先做 1 个 / 都做（分 K 批）"；如实告知成本 K×120 模板 + K 条序列。
